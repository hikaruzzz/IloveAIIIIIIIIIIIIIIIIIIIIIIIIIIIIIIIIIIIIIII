...
#### 3a. skill 类型数据源：自动创建对应 skill

当新增的数据源 `type` 为 `skill` 时，在写入数据源 JSON 后，**必须**同步在本地创建对应的 Claude Code skill：

1. 创建 skill 目录：`.claude/skills/<slug>/`
2. 创建 `SKILL.md` 文件，内容模板：
...
当用户调用 `/share-<slug>` 或需要获取 <市场> 行情数据时，按以下规则执行。
...
3. 告知用户 skill 已创建，可通过 `/share-<slug>` 调用

**注意：** 创建的 SKILL.md 是初始模板，用户后续可根据实际底层数据接口修改其中的获取逻辑。
...
- **如果该数据源 `type` 为 `skill`**：同步删除对应的 skill 目录 `.claude/skills/<slug>/`，并告知用户
- 如果该数据源是当前默认渠道，阻止删除并提示先取消默认或切换默认
